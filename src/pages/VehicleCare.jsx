...
export default function VehicleCare() {
  ...
  const handleProductClick = (productId) => {
    navigate(`/product/Vehicle_care/${productId}`);
  };

  // ✅ Add to Cart functionality (same as in Electronics)
  const handleAddToCart = async (product) => {
    ...
    try {
      await axios.post("http://localhost:3000/cart/add", {
        ...
        category: "vehiclecare", // update category as needed
  ...
  useEffect(() => {
    ...
    const query = categoryParam
      ...
      : "";
    axios
      .get(`http://localhost:3000/vehiclecare?${query}page=${page}&limit=100`)
      .then((res) => {
        setProducts((prev) =>
